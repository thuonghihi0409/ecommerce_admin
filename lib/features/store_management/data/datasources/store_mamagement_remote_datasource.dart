...
abstract class StoreManagementRemoteDatasource {
...
class StoreManagementRemoteDataSourceImpl
    implements StoreManagementRemoteDatasource {
  StoreManagementRemoteDataSourceImpl();

  @override
  Future<ListModel<StoreEntity>> getListStore() async {
    var data = await supabase.from("Stores").select('''
      *,
       user: Users(*)
      ''');

    final result = ListModel(
        results: data.map((product) => StoreEntity.fromJson(product)).toList());
