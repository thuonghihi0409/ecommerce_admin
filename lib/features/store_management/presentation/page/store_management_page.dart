...
class _StoreManagementPageState extends State<StoreManagementPage> {
  ...
  List<StoreEntity> _products = [];

  String _searchQuery = '';
  late StoreManagementBloc _userManagementBloc;
  @override
  void initState() {
    super.initState();
    _userManagementBloc = context.read<StoreManagementBloc>();
    _getDate();
  }

  _getDate() async {
    _userManagementBloc.add(const GetListStore());
  }
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<StoreManagementBloc, StoreManagementState>(
      builder: (context, state) {
        _products = state.listStore.results ?? [];
...
class ProductDataSource extends DataTableSource {
  ...
  final StoreManagementBloc _bloc;
  ProductDataSource(this._products, this._bloc);

  void updateData(List<StoreEntity> newData) {
    _products = newData;
    notifyListeners();
  }

  void sort<T extends Comparable<dynamic>>({
    required T Function(StoreEntity p) getField,
  ...
  DataRow? getRow(int index) {
    ...
    return DataRow(cells: [
      ...
      DataCell(Text(product.user.name, style: AppTextStyles.textSize16())),
      DataCell(
          Text('${product.phone ?? 0}', style: AppTextStyles.textSize16())),
      DataCell(Text(product.status ?? "", style: AppTextStyles.textSize16())),
      DataCell(Text(product.address ?? "", style: AppTextStyles.textSize16())),
      DataCell(Row(
        children: [
          IconButton(
